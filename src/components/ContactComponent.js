...
import { Control, Form, Errors, actions } from 'react-redux-form';  //support validation 

const required = (val) => val && val.length;
const maxLength = (len) => (val) => !(val) || (val.length <= len);
const minLength = (len) => (val) => (val) && (val.length >= len);
const isNumber = (val) => !isNaN(Number(val));                      //make sure it is a number
...
class Contact extends Component {
    ...
    handleSubmit(values) {
        this.props.postFeedback(values.firstname, values.lastname, values.telnum, values.email, 
            values.agree, values.contactType, values.message, this.props.id);
